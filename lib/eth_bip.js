...
EthHelper._calcNonceInPending = function(key, content){
...
ETHBip._getNonce = function(address){
    ...
    // 从queued中计算合适的nonce
    let nonceQueued = EthHelper._calcNonceInQueued(key, queued, transCount);
    if(nonceQueued) {
      return nonceQueued
    }
    // 从pending中计算noncde
    let noncePending = EthHelper._calcNonceInPending(key, pending);
    if(noncePending) {
      return noncePending
    }
    return transCount
