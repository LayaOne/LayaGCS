...
TxPage.onConfirm = function(){
    ...
    //消失的大按钮子
    TxPage.confirm_tx.visible = false;
    TxPage.reject_tx.visible = false;
    TxPage.boradcasting_label.visible = true;


    //todo rawtransaction
    var tx_info = TxPage.current_tx;
    var parmas = tx_info.params[0];
    var rawTx = {}
    rawTx.from = parmas.from;
    rawTx.to = parmas.to;
    rawTx.data = parmas.data;
    ETHBip.signAndSendTx(rawTx,function(err,res){
        console.log(err,res);
        if(err){
            TxPage.boradcasting_label.text = err;
        }
        var HookManager = require('../hook_manager');
        HookManager.on_opreate();
        tx_info.callback(err,res);
    })
